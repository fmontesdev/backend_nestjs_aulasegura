...
export class AccessService {
  ...
  async create(createDto: CreateAccessLogDto): Promise<AccessLogEntity> {
    ...
    accessLog.tagId = createDto.tagId;
  ...
  /// Verifica el acceso de un usuario a un aula y registra el intento
  async checkAccess(checkDto: CheckAccessDto, currentUser: any): Promise<[PermissionEntity | null, AccessStatus, string]> {
    let tagId: number;
    let tagCode: string;
    let userId: string;
    let readerId: number;
